...
def getDistanceTravelled(group):
    respectiveFiles = dystoniaFilesDF['distanceTravelled.npy'].loc[(dystoniaFilesDF['Surgery'] == group[0]) & (dystoniaFilesDF['Genotype'] == group[1]) & (dystoniaFilesDF['Session'] == group[2])]
...
for group in groups:
    ...
    groupFiles = getDistanceTravelled(group)
    ...
    for mice in range(group_n):
        ...
        #miceValues = np.append(arr1, mean_arr2); 
        print(miceValues)
...
# change column names
df_results_per_mice.columns = ['miceID', 'groupSpecs', 'Distance travelled (m)', 'Distance travelled (m/min)']

# display the results
print(df_results_per_mice)

# create a path to save the dataframe containing results for all sessions
filePathResultsPerMice = "H:\\.shortcut-targets-by-id\\1MH0egFqTqTToPE-wxCs7mDWL48lVKqDB\\EurDyscover\\Dystonia_Data\\perSessionAnalysis_distanceTravelled.csv"
